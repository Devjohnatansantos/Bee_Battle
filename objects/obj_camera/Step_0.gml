// deixando em tela cheia
if (keyboard_check_pressed(vk_f8))
{
	window_set_fullscreen(!window_get_fullscreen())
}

if (!instance_exists(obj_player))
{
	exit;
}

// obtendo tamanho da câmera
var _cam_width = camera_get_view_width(view_camera[0]);
var _cam_height = camera_get_view_height(view_camera[0]);

// obtendo as coordenadas
var _cam_x = obj_player.x - _cam_width / 2;
var _cam_y = obj_player.y - _cam_height / 2;

// restringindo a câmera as bordas das rooms
_cam_x = clamp(_cam_x, 0, room_width - _cam_width)
_cam_y = clamp(_cam_y, 0, room_height - _cam_height)

final_cam_x += (_cam_x - final_cam_x) * cam_spd;
final_cam_y += (_cam_y - final_cam_y) * cam_spd;

// setando as coordenadas da câmera 
camera_set_view_pos(view_camera[0], _cam_x, _cam_y);